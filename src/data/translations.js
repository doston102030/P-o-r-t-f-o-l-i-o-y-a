export const translations = {
    uz: {
        nav: {
            home: 'Bosh Sahifa',
            about: 'Haqimda',
            skills: 'Bilimlar',
            projects: 'Loyihalar',
            contact: "Bog'lanish",
            hire: 'Ishga olish',
        },
        hero: {
            badge: 'Ishlashga tayyorman',
            role: 'Frontend Dasturchi (React)',
            tagline: 'Men toza kod va chiroyli interfeyslar orqali zamonaviy va tez ishlovchi veb-ilovalarni yarataman.',
            downloadCv: 'CV yuklab olish',
            contactMe: "Bog'lanish",
            scroll: 'Pastga aylantiring',
            years: '3+ Yil',
            dev: 'React Dev'
        },
        about: {
            tag: 'O‘zim haqimda',
            title: 'Raqamli tajribalar',
            titleGradient: 'yaratish',
            text1: "Salom! Men Abdumajid — Toshkentdanman va men Frontend dasturlashga qiziqaman. Men React ekotizimidan foydalanib zamonaviy, tez va chiroyli veb-ilovalarni yaratishga ixtisoslashganman.",
            text2: "Murakkab muammolarni oddiy va tushunarli dizaynlarga aylantirishni yaxshi ko'raman. Mening maqsadim — toza va tushunarli kod yozish orqali foydalanuvchilarga ajoyib tajriba taqdim etish.",
            highlights: ['Toza Kod', 'Yuqori Tezlik', 'UI/UX Ishqibozi', 'Jamoaviy O‘yinchi'],
            btn: 'Birga Ishlaymiz',
            stats: {
                exp: 'Yillik Tajriba',
                projects: 'Bitgan Loyihalar',
                clients: 'Mamnun Mijozlar',
                satisfaction: 'Qoniqish Darajasi'
            }
        },
        skills: {
            tag: 'Texnologiyalar',
            title: 'Mening',
            titleGradient: 'Texnik Bilimlarim',
            subtitle: 'Zamonaviy va tez ishlovchi veb-ilovalarni yaratishda foydalanadigan texnologiyalarim'
        },
        projects: {
            tag: 'Portfolio',
            title: 'Tanlangan',
            titleGradient: 'Loyihalar',
            subtitle: 'G‘oyadan to natijagacha — mening so‘nggi ishlarim',
            featured: 'Tanlangan',
            demo: 'Jonli Demo',
            github: 'Kodini Ko‘rish'
        },
        contact: {
            tag: "Bog'lanish",
            title: "Keling",
            titleGradient: 'Birga Ishlaymiz',
            subtitle: "Biror loyiha bormi? Men bilan bog'laning.",
            infoTitle: "Aloqa Ma'lumotlari",
            infoText: "Men frilans loyihalar va doimiy ish uchun ochiqman. Yozing va ajoyib narsa yarataylik!",
            form: {
                name: 'Ismingiz',
                email: 'Email Manzilingiz',
                message: 'Xabaringiz',
                send: 'Xabar Yuborish',
                success: 'Xabar Yuborildi!',
                successText: "Bog'langaningiz uchun rahmat. Tez orada javob beraman!"
            }
        },
        footer: {
            tagline: 'Kelajakni har bir komponent orqali quramiz.',
            rights: 'Barcha huquqlar himoyalangan.',
            made: 'Toshkentda mehr bilan yaratildi ❤️'
        }
    },
    ru: {
        nav: {
            home: 'Главная',
            about: 'Обо мне',
            skills: 'Навыки',
            projects: 'Проекты',
            contact: 'Контакты',
            hire: 'Нанять меня',
        },
        hero: {
            badge: 'Открыт к предложениям',
            role: 'Frontend Разработчик (React)',
            tagline: 'Я создаю современные и быстрые веб-приложения с чистым кодом и красивым интерфейсом.',
            downloadCv: 'Скачать CV',
            contactMe: 'Связаться',
            scroll: 'Листайте вниз',
            years: '3+ Года',
            dev: 'React Dev'
        },
        about: {
            tag: 'Обо мне',
            title: 'Создаю цифровой',
            titleGradient: 'опыт',
            text1: "Привет! Меня зовут Абдумажид — я увлеченный Frontend-разработчик из Ташкента. Я специализируюсь на создании современных, производительных веб-приложений с использованием React.",
            text2: "Я люблю превращать сложные задачи в простые и интуитивно понятные интерфейсы. Моя цель — писать чистый, поддерживаемый код и создавать отличный пользовательский опыт.",
            highlights: ['Чистый Код', 'Производительность', 'UI/UX Энтузиаст', 'Командный Игрок'],
            btn: 'Давайте Работать Вместе',
            stats: {
                exp: 'Лет Опыта',
                projects: 'Проектов',
                clients: 'Довольных Клиентов',
                satisfaction: 'Уровень Удовлетворенности'
            }
        },
        skills: {
            tag: 'Стек Технологий',
            title: 'Мои',
            titleGradient: 'Навыки',
            subtitle: 'Технологии, которые я использую для создания современных веб-приложений'
        },
        projects: {
            tag: 'Портфолио',
            title: 'Избранные',
            titleGradient: 'Проекты',
            subtitle: 'От идеи до реализации — мои последние работы',
            featured: 'Избранное',
            demo: 'Смотреть Демо',
            github: 'Смотреть Код'
        },
        contact: {
            tag: 'Контакты',
            title: 'Давайте',
            titleGradient: 'Сотрудничать',
            subtitle: 'Есть идея проекта? Я буду рад обсудить.',
            infoTitle: 'Контактная Информация',
            infoText: 'Я открыт для фриланс-проектов и постоянной работы. Свяжитесь со мной!',
            form: {
                name: 'Ваше Имя',
                email: 'Ваш Email',
                message: 'Сообщение',
                send: 'Отправить',
                success: 'Сообщение Отправлено!',
                successText: 'Спасибо за обращение. Я скоро отвечу!'
            }
        },
        footer: {
            tagline: 'Создаем будущее, компонент за компонентом.',
            rights: 'Все права защищены.',
            made: 'Сделано с любовью в Ташкенте ❤️'
        }
    },
    en: {
        nav: {
            home: 'Home',
            about: 'About',
            skills: 'Skills',
            projects: 'Projects',
            contact: 'Contact',
            hire: 'Hire Me',
        },
        hero: {
            badge: 'Available for work',
            role: 'Frontend Developer (React)',
            tagline: 'I build modern and fast web applications with clean code and beautiful interfaces that users love.',
            downloadCv: 'Download CV',
            contactMe: 'Contact Me',
            scroll: 'Scroll down',
            years: '3+ Years',
            dev: 'React Dev'
        },
        about: {
            tag: 'About Me',
            title: 'Crafting digital',
            titleGradient: 'experiences',
            text1: "Hi! I'm Abdumajid — a passionate Frontend Developer from Tashkent, Uzbekistan. I specialize in building modern, performant, and visually compelling web applications using React ecosystem technologies.",
            text2: "I love turning complex problems into simple, beautiful, and intuitive designs. My goal is to write clean, maintainable code while delivering outstanding user experiences that make a real impact.",
            highlights: ['Clean Code Advocate', 'Performance-first', 'UI/UX Enthusiast', 'Team Player'],
            btn: "Let's Work Together",
            stats: {
                exp: 'Years Experience',
                projects: 'Projects Done',
                clients: 'Happy Clients',
                satisfaction: 'Satisfaction Rate'
            }
        },
        skills: {
            tag: 'What I work with',
            title: 'My',
            titleGradient: 'Tech Stack',
            subtitle: 'Technologies I use to build modern, performant web applications'
        },
        projects: {
            tag: 'Portfolio',
            title: 'Featured',
            titleGradient: 'Projects',
            subtitle: 'A selection of my recent work — from concept to deployment',
            featured: 'Featured',
            demo: 'Live Demo',
            github: 'GitHub'
        },
        contact: {
            tag: 'Get in touch',
            title: "Let's",
            titleGradient: 'Work Together',
            subtitle: "Have a project in mind? I'd love to hear about it.",
            infoTitle: 'Contact Information',
            infoText: "I'm currently open to freelance projects and full-time opportunities. Reach out and let's build something amazing together!",
            form: {
                name: 'Your Name',
                email: 'Email Address',
                message: 'Message',
                send: 'Send Message',
                success: 'Message Sent!',
                successText: "Thanks for reaching out. I'll get back to you soon!"
            }
        },
        footer: {
            tagline: 'Building the future, one component at a time.',
            rights: 'All rights reserved.',
            made: 'Made with ❤️ in Tashkent, Uzbekistan'
        }
    }
};
